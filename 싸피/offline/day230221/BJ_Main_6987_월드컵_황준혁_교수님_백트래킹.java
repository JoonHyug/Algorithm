...
public class BJ_Main_6987_월드컵_황준혁_교수님_백트래킹 {
	...
	static boolean isValid; // 현재 경기결과가 유효하다면 true
	...
	private static void bruteForce(int cnt) {
		// 모든 경기결과를 경정했다면
		if (cnt == 15) {
			// 만들어진 경기결과와 주어진 경기결과의 모든 승, 무, 패가 일치한다면 유효
			//이미 가능한 경우만 도착했기 때문에 배열을 비교해줄 필요가 없다.
//			for (int i = 0; i < result.length; i++) {
//				for (int j = 0; j < result[i].length; j++) {
//					if (result[i][j] != madeResult[i][j])
//						return;
//				}
//			}
